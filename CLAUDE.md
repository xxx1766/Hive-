...
| Agent 清单文件 | **`agent.yaml`** | 单个 Agent 的 manifest（name/version/kind/entry/rank/quota） | Dockerfile |
| Room 声明文件 | **Hivefile** (`*.yaml`) | 一个 Room 招哪些 Agent、如何覆盖 Rank / 配额 | docker-compose.yaml |
...
| 构建 | `hive build <dir>` | 按 Agent 源码目录里的 `agent.yaml` 打包 Image | `docker build` |
| 编排 | `hive hire -f <hivefile>` | 按 Hivefile 拉起 Room + 批量招聘所有声明的 Agent | `docker compose up` |
